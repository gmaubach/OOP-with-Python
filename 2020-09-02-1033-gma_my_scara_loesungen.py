# -*- coding: utf-8 -*-
"""
Created on Mon Sep 2 09:16:45 2020
...
def geheSchritte(anzahl):
    x = 1;
    while x <= anzahl:
        Scara.gehe();
        x += 1;

def loeseFesteEntfernung2():
    Scara.starte(b"FesteEntfernung");
...
def loeseAllesMussWeg():                         # LÄUFT !!!
    Scara.starte(b"AllesMussWeg");

    while (Scara.stehtAufAnkh() == False):
        geheBisAbgrund();
        dreheRechts();
        Scara.gehe();
        dreheRechts();
        geheBisAbgrund();
        if (Scara.stehtAufAnkh() == False):
            Scara.dreheLinks();
            Scara.gehe();
            Scara.dreheLinks();
...
def loeseEntfernungsmesser():                    # LÄUFT !!!
    Scara.starte(b"Entfernungsmesser");

    schritte = 0;

    while (Scara.stehtAufTerminal() == False):
        Scara.gehe();
        schritte += 1;
    
    Scara.setzeTerminalwert(schritte);
    print("Entfernung: ", schritte);

    dreheUm();
    geheSchritte(schritte);

    Scara.beende();

def loeseEntfernungsmesser_Musterloesung(): 
    Scara.starte(b"Entfernungsmesser") 
 
    schritte = 0 
    while not Scara.istVorneAbgrund(): 
        Scara.gehe() 
        schritte += 1 
...
def loeseEntfernungsmesser2():                   # LÄUFT !!!
    Scara.starte(b"Entfernungsmesser") ;

    schritte = geheWeitmoeglichst();
    Scara.setzeTerminalwert(schritte);
    dreheUm();
    geheSchritte(schritte);
...
def loeseEntfernungsmesser2_Musterloesung(): 
    Scara.starte(b"Entfernungsmesser") 
